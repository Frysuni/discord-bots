...
export class EntityTable {
    ...
    public remove(where: FindOptionsWhere<ObjectLiteral> | FindOptionsWhere<ObjectLiteral>[]) {
        return this.table.findOneBy(where)
                .then(record => record ? this.table.remove(record) : null);
    }

    /**
     * Найти 1 элемент в таблице.
     */
    public find(where: FindOptionsWhere<ObjectLiteral> | FindOptionsWhere<ObjectLiteral>[]) {
        return this.table.findOneBy(where);
    }
}
